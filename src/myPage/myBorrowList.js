...
import { Card, Col, Row, Button, Modal, Input, Form, Rate } from "antd";
...
function MyBorrowListPage() {
  ...
  const onReset = () => {
    form.resetFields();
    setReviewScoreReset(reviewScore);
  };
  ...
  function setReviewScoreReset(reviewScore) {
    setReviewScore(3);
  }
          ...
          {myBorrowItemList.map((borrowitem, index) => {
            return (
              <Col span={4.5} className="col">
                <Card hoverable key={borrowitem.itemId} className="cards">
                  <span>
                    {" "}
                    <h2
                      className="title"
                      onClick={() => {
                        toProductViewDetailsPage(borrowitem.itemId);
                      }}
                    >
                      제목: {borrowitem.itemTitle}
                    </h2>
              ...
              <div className="reviewText">
                ...
                <p>
                  ...
                  {myBorrowItemList.length == 0
                    ? ""
                    : format(
                        new Date(myBorrowItemList[currentIndex].startDate)
                      )}
                </p>
                <p>
                  대여료:{" "}
                  {myBorrowItemList.length == 0
                    ? ""
                    : myBorrowItemList[currentIndex].price}
                </p>
                <p>
                  보증금:{" "}
                  {myBorrowItemList.length == 0
                    ? ""
                    : myBorrowItemList[currentIndex].deposit}
                </p>
              </div>
            </row>
          </Card>
