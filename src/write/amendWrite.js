...
import { TagsFilled, UploadOutlined } from "@ant-design/icons";
...
function AmendWritePage() {
      ...
      .then((response) => {
        ...
        photos.map((photo) => {
          fetch(photo.itemPhoto)
            ...
            .then((blob) => {
              const ext = photo.itemPhoto.split(".").pop(); // url 구조에 맞게 수정할 것
              const filename = photo.itemPhoto.split("/").pop(); // url 구조에 맞게 수정할 것
  ...
  const setContentChange = (e) => {
    setContent(e.target.value);
  };
  const setPriceChange = (e) => {
    setPrice(e.target.value);
  };
  const setDepositChange = (e) => {
  ...
  const onSubmit = () => {
    ...
    imageList.forEach((image) =>
      formData.append("itemPhoto", image.originFileObj)
    );
    ...
    <div className="write">
      ...
      >
        <Form>
          <h1 className="h1">
            {" "}
            {userNickName} 님의 물품 빌려 주기 <TagsFilled />{" "}
          </h1>
          ...
          <div className="write_wrap">
            ...
            <Form.Item
              className="write_money"
              name="price"
              rules={[{ required: true, message: "대여금을 입력해주세요!" }]}
            >
              <Input
                type="number"
                placeholder="대여금을 입력해주세요"
          ...
          <div className="write_wrap">
            ...
            <Form.Item className="itemPhoto" name="itemPhoto">
              <ImgCrop rotate>
                <Upload
                  listType="picture"
                  fileList={imageList}
          ...
          <div className="write_wrap">
            ...
            <Form.Item name="address">
              ...
              <Modal
                ...
                onCancel={modalClose}
              >
                <DaumPostCode onComplete={getAddress} autoClose={false} />
                {address}
              </Modal>
              <br />
              {address}
          ...
          <div className="write_wrap">
            ...
            <Form.Item name="quality" initialValue={quality}>
              <label>상</label>
              &nbsp;
              <Input
                type="radio"
